...
def train_reader(train_list, buffered_size=5120):
    print "inside train_reader"
    def reader():
        with open(train_list, 'r') as f:
            lines = [line.strip() for line in f.readlines()]
            print "samples num: {0}".format(len(lines))
            for line in lines:
                try:
...
if __name__ == '__main__':
    #for im in train_reader('train.lst'):
    #    print len(im[0])
    train_reader('train.lst')
    #for im in train_reader('val.lst'):
    #    print len(im[0])
