...
def conv_bn_layer(input,
    ...
    tmp = paddle.layer.img_conv(
        ...
        bias_attr=False)
    return paddle.layer.batch_norm(input=tmp, act=active_type, param_attr=param_attr)
...
def basicblock(input, ch_out, stride):
    short = shortcut(input, ch_out, stride, param_attr=param_attr)
    conv1 = conv_bn_layer(input, ch_out, 3, stride, 1)
    conv2 = conv_bn_layer(conv1, ch_out, 3, 1, 1, paddle.activation.Linear())
    return paddle.layer.addto(
        input=[short, conv2], act=paddle.activation.Relu()tr)
